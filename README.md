...
- **`e2e/claim.spec.ts`** — the admin-reviewed claim flow: an admin adds an
  unclaimed listing, the real owner submits a claim request with a proof
  document, the admin approves it, and the owner sets a password and lands
  on their dashboard. The only spec that needs the seeded `admin@example.com`
  account (see `db/seed.ts`), since there's no self-serve way to become an
  admin.
...
- **Map, folded into Browse** — `/map` used to be its own page; it's now a collapsible section at the top of `/browse`'s Browse tab (web) / the Browse tab of mobile's home page, above the filters — `lib/components/TruckMap.svelte` plots every truck the *current filters* returned (search/cuisine/open-now/dietary/sort all apply to the map too, since it's fed the same `trucks` array as the list below it), plus a blue dot for your own location via a "Center on me" button (or automatically once you use "Near me"/"Sort: nearest"). The collapse state persists per-browser via `localStorage` (`lft:browse-map-collapsed`) — a per-viewer convenience, not synced anywhere. `/map` itself now just redirects to `/browse` for old links. Tiles come from Esri's public World Street Map service via a plain MapLibre raster style (`lib/map.ts`, shared by `TruckMap.svelte` and `LocationPickerModal.svelte` so there's one place to swap it) — no API key needed, no watermark, and unlike OSM's own tile server, it's explicitly meant to be used this way by other sites' production traffic (CARTO's free basemaps were tried first; despite docs describing them as keyless, they now overlay an "API KEY" watermark without one — found by actually loading a tile, not just reading the docs). Swap the source for a vector style from MapTiler, Stadia Maps, etc. (with your own key) if you outgrow Esri's free tier. `TruckMap` takes an optional `height` prop (defaults to a taller `70vh`/`65vh` for its plain-map days, now usually passed a smaller value — `40vh` in Browse, `50vh` in the truck-detail modal below) so the same component works everywhere it's used.
...
- **Admin dashboard** — `/admin` (web only; there's no admin surface in the mobile app) gates on `users.isAdmin`, a boolean orthogonal to the truck/customer `role` column — there's no self-serve way to become one, it's set directly in the database (`db/seed.ts` seeds one demo admin — see the Demo accounts table). `apps/api/src/routes/admin.ts` is the API side (`requireAdmin` middleware, `lib/auth.ts`); the page itself has a Trucks tab (verify/unverify — sets `truck_profiles.verified`, shown as a blue check badge next to the truck's name everywhere it's displayed), a Claim requests tab (see below), and a Reviews tab (hide/unhide — sets `reviews.hidden_at`, a soft moderation flag: a hidden review is excluded from the truck's public review list and from its `averageRating`/`reviewCount`, but stays in the database rather than being deleted). Enforced server-side (`requireAdmin` on every `/api/admin/*` route, independent of the client); the page itself also redirects a non-admin away client-side, but that's a UX nicety, not the actual gate.
- **Admin-added trucks and claiming them** — the Trucks tab's "Add truck" form lets an admin create a listing for a truck that hasn't signed up itself (name/cuisine/description/contact info). This still creates a full auth user + `users` + `truck_profiles` row immediately — everything else in the schema FKs to `users.id`, so a truck "existing" without one isn't an option — just with a placeholder `@trucks.invalid` email and a random password nobody is told, so the account exists but can't be signed into (`truck_profiles.claimedAt` is null until it actually is). The admin gets back a `/claim?token=...` link (`truck_profiles.claimToken`, 14-day expiry) to send the real owner however they've already verified reaches them, or — if they typed an owner email into the form — it's emailed automatically (`lib/email.ts#sendClaimTruckEmail`, same Resend/MailDev transport as password-reset). That link **doesn't** hand over the account: it opens `/claim`, where whoever holds it submits a claim request — name, email, phone, an optional note, and a **required** photo/PDF of a seller's permit or business license (`POST /api/claim/:token`, multipart, uploaded to a private `claim-documents` Supabase Storage bucket — unlike every other upload in the app, this one is never public; see `lib/uploads.ts`). Nothing about account access changes yet. An admin reviews it on the Claim requests tab (proof document shown via a 10-minute signed URL, generated fresh per fetch — `getClaimDocumentUrl`) and approves or denies it; several people can have a pending request for the same truck at once (e.g. someone else grabbed the shared link too), and approving one auto-denies the rest for that truck. Approving emails (or, same dev/no-email-service fallback as everywhere else, hands back directly) a second, separate link — `/claim/finish?token=...`, a fresh 7-day `truck_claim_requests.setPasswordToken`, unrelated to the first token — where the approved requester sets a password. Only *that* step (`POST /api/claim/finish/:token`) actually swaps the placeholder email/password for real ones via `supabaseAdmin.auth.admin.updateUserById`, flips `claimedAt`, and signs them in. Two tokens, two purposes: the first only proves "an admin sent me this," the second proves "an admin approved *me* specifically" — collapsing them into one step would mean anyone who intercepts the shareable first link gets the account outright.
