<script lang="ts">
	...
	import { addWatchLocation, deleteWatchLocation, getAuth, signOut, updateAccount, uploadAvatar } from '$lib/auth.svelte';
	import { client } from '$lib/api';
	import Avatar from '$lib/components/Avatar.svelte';
	import ImageCropModal from '$lib/components/ImageCropModal.svelte';
	import { getCurrentCoords } from '$lib/geo';
	import { listTimezones } from '$lib/time';
	import { GpsFix, MapPin, SignOut, Trash } from 'phosphor-svelte';
	...
	function windowSummary(loc: WatchLocationRecord): string {
		...
		const days =
			loc.activeDays.length === 0 || loc.activeDays.length === 7
				? 'Every day'
...
{#if auth.session}
	...
	<button
		class="flex w-full items-center justify-center gap-1.5 rounded border border-stone-300 px-4 py-2 text-stone-600 hover:bg-stone-100"
		onclick={() => signOut()}
	>
