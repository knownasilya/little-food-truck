<script lang="ts">
	...
	// Served by the API (apps/api/src/routes/share.ts), not this app — it's
	// the one part of this stack with a real runtime server, so the preview
	// is always current even for a truck created seconds ago.
...
{:else}
	{#if truck.photoUrl}
		<img src={resolveUploadUrl(truck.photoUrl)} alt="" class="mb-4 h-40 w-full rounded-lg object-cover" />
	{/if}
	<div class="rounded-lg border border-stone-200 bg-white p-4">
		<div class="flex items-start justify-between gap-4">
			<div>
				<div class="flex items-center gap-1.5">
					<h1 class="text-xl font-bold">{truck.name}</h1>
					{#if truck.verified}
						<SealCheck size={18} weight="fill" class="shrink-0 text-blue-500" aria-label="Verified" />
					{/if}
				</div>
		...
		<div class="mt-3 flex flex-wrap items-center gap-3 text-sm text-stone-500">
			<span class="flex items-center gap-1"><Heart size={16} weight="fill" /> {truck.favoriteCount}</span>
			{#if truck.averageRating != null}
				<span class="flex items-center gap-1 text-amber-600">
					<Star size={16} weight="fill" /> {truck.averageRating.toFixed(1)} ({truck.reviewCount})
				</span>
			{/if}
		...
		{#if auth.session?.role === 'customer'}
			<button
				class="mt-4 flex w-full items-center justify-center gap-1.5 rounded border border-orange-300 px-4 py-2 text-orange-700 hover:bg-orange-50"
		...
		{#if auth.session && auth.session.role !== 'truck' && truck.claimed}
			<button
				class="mt-2 flex w-full items-center justify-center gap-1.5 rounded border border-stone-300 px-4 py-2 text-stone-700 hover:bg-stone-100"
		...
		{#if !truck.claimed && auth.session?.role !== 'truck'}
