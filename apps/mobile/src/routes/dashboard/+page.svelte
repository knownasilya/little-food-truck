<script lang="ts">
	...
	import { getCurrentCoords, hasLocationService as checkLocationService } from '$lib/geo';
	...
	onMount(() => {
		checkLocationService().then((available) => (hasLocationService = available));
	});
	...
	async function updateLocation(isOpen: boolean) {
		...
		try {
			...
			try {
				...
				await postLocation({ lat: coords.lat, lng: coords.lng, isOpen: true });
			} catch {
...
{#if auth.session}
	<section class="mb-6 rounded-lg border border-stone-200 bg-white p-4">
		...
		<p class="mb-3 text-sm text-stone-500">
			Go open when you're set up and serving — favoriters nearby get notified.
		</p>
		<div class="flex flex-col gap-2">
			<button
				disabled={updatingLocation}
				class="flex items-center justify-center gap-1.5 rounded bg-green-600 px-4 py-2 text-white hover:bg-green-700 disabled:opacity-50"
