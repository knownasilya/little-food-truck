		...
		try {
			...
			if (nearMe || sort === 'distance') {
				try {
					...
					query.lat = String(coords.lat);
					query.lng = String(coords.lng);
					query.radiusMiles = nearMe ? '10' : '500';
					userLocation = coords;
				} catch {
					error = 'Could not get your location. Check location permissions.';
					return;
				}
			}
...
{#if !auth.loading && !auth.session}
	<div class="mb-4 flex items-center justify-between rounded-lg bg-orange-50 p-3 text-sm">
		<span>Sign in to favorite trucks and get nearby alerts.</span>
...
<div class="mb-4 flex gap-1 overflow-x-auto border-b border-stone-200">
	...
	>
		<Sparkle size={16} weight="fill" /> New
...
{#if activeTab === 'browse'}
	<div class="mb-4">
		...
		{#if mapExpanded}
			...
			{/if}
			<TruckMap trucks={mapTrucks} {userLocation} height="35vh" />
		{/if}
	</div>
	...
	<div class="mb-4 flex flex-col gap-2">
		...
		<div class="flex gap-2">
			...
			<button
				class="flex items-center gap-1 rounded border px-3 py-2 text-sm {nearMe
