<script lang="ts">
	...
	// real push notifications, which would need APNs/FCM wiring on mobile.
	const interval = setInterval(load, 30_000);
	onDestroy(() => clearInterval(interval));
</script>

<svelte:head><title>Notifications — Little Food Truck</title></svelte:head>
...
{:else if notifications.length === 0}
	<p class="text-stone-400">
		Nothing yet. Favorite a truck and set your home location in your profile to get alerted when
		it's nearby.
	</p>
{:else}
