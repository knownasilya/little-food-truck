<script lang="ts">
	...
	async function onSubmit(e: SubmitEvent) {
		...
		try {
			...
			await goto(role === 'truck' ? '/dashboard' : '/browse');
		} catch (err) {
...
<svelte:head><title>Sign up — Little Food Truck</title></svelte:head>
...
<div class="mx-auto max-w-sm">
	...
	<div class="mb-4 flex rounded border border-stone-300 p-1 text-sm">
		<button
			type="button"
			class="flex-1 rounded px-3 py-1.5 {role === 'customer'
		...
		<button
			type="button"
			class="flex-1 rounded px-3 py-1.5 {role === 'truck'
				? 'bg-orange-600 text-white'
				: 'text-stone-600'}"
			onclick={() => (role = 'truck')}
		>
	...
	<p class="mt-3 text-xs text-stone-400">
		By creating an account you agree to the <a href="/terms" class="underline hover:text-stone-600">Terms</a>
		and <a href="/privacy" class="underline hover:text-stone-600">Privacy Policy</a>.
	</p>
